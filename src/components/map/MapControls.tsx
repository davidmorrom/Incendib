...
export function MapControls({
  ...
  return (
    <div className="absolute right-[10px] top-[10px] z-[3] flex flex-col gap-2">
      <button
        type="button"
        className={cn(btn, layersActive ? 'text-action-text' : 'text-fg-secondary')}
      ...
      <button type="button" className={cn(btn, 'text-fg-secondary')} onClick={onLocate} aria-label={d.map.locateAria} title={d.map.locateAria}>
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.3">
