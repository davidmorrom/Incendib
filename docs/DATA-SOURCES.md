...
**Portugal — ANEPC (oficial) + fogos.pt (respaldo) + ICNF.**
- **ANEPC / SGIFR (FUENTE PRIMARIA, oficial)** — FeatureServer público de ocorrências facilitado a Incendib por la **AGIF** y el **ICNF** en respuesta a nuestra solicitud (jul 2026). Es el mismo servicio que alimenta el SIFOR/GeoSIFOR (`https://www.sgifr.gov.pt/fogos-incendios-rurais-ativos`, `https://geosifor.sgifr.gov.pt`) y la app de ocorrências de la ANEPC — la **misma fuente subyacente que fogos.pt** (Sistema de Informação Operacional da ANEPC), pero desde el endpoint autoritativo y **sin registro**. Endpoint: `https://services-eu1.arcgis.com/VlrHb7fn5ewYhX6y/ArcGIS/rest/services/OcorrenciasSite/FeatureServer/0/query`. Filtro por `CodNatureza` (indicado por la AGIF): `3101` povoamentos florestais, `3103` matos, `3105` áreas agrícolas = **incendios reales**; `3107` rescaldo, `3109` fogo controlado, `3111` queima, `4335` prevenção = NO incendios. Campos ricos: `EstadoOcorrencia`/`EstadoAgrupado`, `Concelho`/`Freguesia`/`Regiao`/`SubRegiao`, `Operacionais`/`MeiosTerrestres`/`MeiosAereos`, `Latitude`/`Longitude`, `DataOcorrencia`. **Condiciones (AGIF/ICNF):** la BD se actualiza ~cada 10 min; sin límite de peticiones configurado pero se congestiona en picos → cachear del lado servidor y consultar con moderación. **Atribución: ANEPC (`https://prociv.gov.pt`)**, NO AGIF ni fogos.pt para este servicio. Para perímetros de grandes incendios, la ANEPC (contacto institucional `geral@prociv.pt`) puede registrar la evolución del perímetro. El feed NO publica superficie (el área solo se consolida tras la extinción, vía ICNF).
...
| Portugal activos (primaria) | `services-eu1.arcgis.com/VlrHb7fn5ewYhX6y/…/OcorrenciasSite/FeatureServer/0/query` (filtro `CodNatureza IN (3101,3103,3105)`) | ArcGIS GeoJSON/JSON | **Oficial (ANEPC/SGIFR, atribución a prociv.gov.pt)** |
| Portugal activos (respaldo) | `api.fogos.pt/v2/incidents/active` | JSON | No oficial (VOST, datos ANEPC, refresco 2 min) |
